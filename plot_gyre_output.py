...
def plot_gyre_div_vor_tests(exp):
    ...
    units = '$m^{-1}$'
    plt.pcolormesh(lon, lat, psi_div, cmap='viridis', shading='auto')#, vmin=-1, vmax=1)
    plt.xlabel('Longitude ($^\circ$)')
    plt.ylabel('Lattitude ($^\circ$)')
    ...
    plt.title(f'Divergence of the velocity generated by streamfunction')
    units = '$m^{-1}$'
    plt.pcolormesh(lon[:30, :30], lat[:30, :30], psi_div[:30, :30], cmap='viridis', shading='auto')  # , vmin=-1, vmax=1)
    plt.xlabel('Longitude ($^\circ$)')
    plt.ylabel('Lattitude ($^\circ$)')
    plt.colorbar(label=f'Divergence ({units})')
    plt.show()
